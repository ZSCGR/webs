...
function updateInfo(t) {
    ...
    '</span>';
    document.getElementById('cost').innerHTML = '花费: $' + t.totalCost;
    document.getElementById('sellPrice').innerHTML = '出售价格: $' +
    t.sellPrice();
    document.getElementById('upPrice').innerHTML = '升级价格: ' +
    (t.upgrades.length > 0 ? '$' + t.upgrades[0].cost : 'N/A');
    document.getElementById('damage').innerHTML = '伤害: ' + t.getDamage();
    document.getElementById('type').innerHTML = '类型: ' +
    t.type.toUpperCase();
    document.getElementById('range').innerHTML = '范围: ' + t.range;
    document.getElementById('cooldown').innerHTML = '平均值 冷却时间: ' +
...
function updatePause() {
    document.getElementById('pause').innerHTML = paused ? '开始' : '暂停';
}

// Update game status display with wave, health, and cash
function updateStatus() {
    document.getElementById('wave').innerHTML = '第 ' + wave +' 波';
    document.getElementById('health').innerHTML = '生命值: ' +
    health + '/' + maxHealth;
